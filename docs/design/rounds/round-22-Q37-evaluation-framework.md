...
If mixed, run the remaining 4. Budget: 8 tasks x ~$0.50 vaglio cost + ~$0.12
...
`[inferred]` With 12 tasks x 4 modes (vaglio + 3 baselines) = 48 runs.
...
Pragmatic recommendation: start with 6 tasks x 2 modes (vaglio vs.
...
**Phased approach:** Start with 6 tasks x vaglio vs. protocol-structured single
(the most informative comparison). If vaglio wins clearly, expand to full 12
tasks x all baselines. Budget: $7-20.
