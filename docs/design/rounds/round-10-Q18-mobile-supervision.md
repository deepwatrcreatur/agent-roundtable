...
- Repo moved from `sst/opencode` to `anomalyco/opencode`. 779 releases; latest April 27 2026. Fast-moving upstream.
...
The repo is extremely fast-moving (779 releases, latest April 27 2026, already migrated from `sst/opencode` to `anomalyco/opencode`). A fork tracking that velocity plus maintaining a Swift client plus $99/year Apple Dev plus 90-day TestFlight build refreshes is a non-trivial ongoing cost for partial fit.
